...
document.addEventListener("DOMContentLoaded", function () {
	...
	while (perm.length < 255) {
		perm.includes(val = Math.floor(Math.random()*255));
		perm.push(val);
	}
	...
	let player = new function() {
		...
		this.draw = function() {
			...
			let andle = Math.atan2((p2 - 28) - this.y, (this.x + 5) - this.x);
			this.y += this.ySpeed;

			if (grounded && playing) {
				this.rot -= (this.rot - andle) * 0.5;
				this.rSpeed = this.rSpeed - (andle - this.rot);
			}
			...
			ctx.drawImage(this.img, -28, -28, 60, 60);
			ctx.restore();
		}
	}
